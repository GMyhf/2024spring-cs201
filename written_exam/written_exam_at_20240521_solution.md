...
（11）设F是一个森林，B是由F变换得的二叉树。若F中有n个非终端结点，则B中右指针域为空的结点有（ C ）个。
...
#对于二叉搜索树（BST），如果树是平衡的，那么在平均情况下，查找、插入和删除的时间复杂度也是 𝑂(log⁡𝑛)*O*(log*n*)。但是，如果树不平衡，例如退化为链表，那么时间复杂度可能会变为 𝑂(𝑛)*O*(*n*)。
...
在最大堆中，对于任意节点 𝑖*i*，其父节点 parent(𝑖)parent(*i*) 的值大于等于 𝑖*i* 节点的值。在最小堆中，对于任意节点 𝑖*i*，其父节点 parent(𝑖)parent(*i*) 的值小于等于 𝑖*i* 节点的值。
